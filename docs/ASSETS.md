...
| 종류 | registry | 없을 때 |
|---|---|---|
| 플레이어 캐릭터 2D | `src/config/character-assets.ts` → `PlayerCharacterAssets` | 중립 도형 실루엣 |
...
`home` 하나만 채워도 나머지 슬롯이 그걸 쓴다 (`resolveCharacterAsset`).
...
연결 순서는 `src/components/character/character-3d-viewer.ts`의 `TODO(character-3d)` 참고.
...
1. **플레이어 캐릭터 2D** (`PlayerCharacterAssets.home`) — HOME/HISTORY/RESULT가 한 번에 바뀐다

...
const insufficient: DanbaekLearningProfile = {
  ...
  capabilities: [{ movementFamily: 'push_horizontal', learningStage: 'imitating', evidenceCount: 2, lastObservedAt: '2026-08-25T00:00:00.000Z', representativeExerciseIds: ['bench-press'] }],
};
const learnedWithoutBench: DanbaekLearningProfile = {
  ...insufficient,
  capabilities: [{ ...insufficient.capabilities[0], learningStage: 'learned', evidenceCount: 3, representativeExerciseIds: ['push-up'] }],
};
const learnedBench: DanbaekLearningProfile = {
  ...learnedWithoutBench,
  capabilities: [{ ...learnedWithoutBench.capabilities[0], evidenceCount: 4, representativeExerciseIds: ['push-up', 'bench-press'] }],
};
...
if (secondRun.block) check('specific exercise id survives handoff', presentDanbaekWorldBlock(secondRun.block).specificExerciseId === 'bench-press');

const malformedCrossFamily: DanbaekLearningProfile = {
  ...learnedWithoutBench,
  capabilities: [learnedWithoutBench.capabilities[0], { movementFamily: 'pull_horizontal', learningStage: 'proficient', evidenceCount: 99, lastObservedAt: '2026-08-25T00:00:00.000Z', representativeExerciseIds: ['bench-press'] }],
};
...
if (benchGate) check('specific exercise in wrong movement family cannot unlock gate', evaluateDanbaekWorldStage(benchGate, malformedCrossFamily).outcome === 'block');

...
const learningRank = new Map<LearningStage, number>(LearningStages.map((stage, index) => [stage, index]));
...
export function evaluateDanbaekWorldStage(stage: DanbaekWorldStage, profile: DanbaekLearningProfile): StageEvaluation {
  ...
  const stageSatisfied = requirement.minimumLearningStage
    ...
    : true;
  const exerciseCandidates = requirement.movementFamily ? (capability ? [capability] : []) : profile.capabilities;
  const exerciseSatisfied = requirement.specificExerciseId
    ...
    : true;
  if (familySatisfied && stageSatisfied && exerciseSatisfied) return { outcome: 'pass', stageId: stage.id };
  return {
    outcome: 'block',
    stageId: stage.id,
    requirement,
    recommendedMovementFamily: requirement.movementFamily ?? capability?.movementFamily ?? 'locomotion',
    explanationKey: requirement.specificExerciseId ? 'world.block.specific_exercise_required' : 'world.block.learning_required',
  };
}

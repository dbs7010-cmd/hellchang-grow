...
export function runDanbaekAdventure(stages: readonly DanbaekWorldStage[], profile: DanbaekLearningProfile): DanbaekAdventureRun {
  ...
  for (const stage of stages) {
    ...
    if (evaluation.outcome === 'block') {
      return { visitedStageIds, clearedStageIds, currentStageId: stage.id, outcome: 'blocked', block: evaluation };
    }

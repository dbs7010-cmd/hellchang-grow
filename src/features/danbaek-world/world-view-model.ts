...
export type DanbaekWorldSceneState =
  ...
  | { state: 'blocked'; title: string; body: string; stageId: string; clearedStageIds: string[]; actionLabel: string; recommendedMovementFamily: MovementFamily; specificExerciseId: string | null };
...
export function buildDanbaekWorldScene(profile: DanbaekLearningProfile): DanbaekWorldSceneState {
  ...
  if (run.outcome === 'cleared') {
    return { state: 'cleared', title: '단백이가 길을 열었어요', body: '배운 동작으로 이번 길을 끝까지 지나갔습니다.', clearedStageIds: run.clearedStageIds };
  }
  const presentation = presentDanbaekWorldBlock(run.block!);
  return { state: 'blocked', title: presentation.title, body: presentation.body, stageId: run.currentStageId!, clearedStageIds: run.clearedStageIds, actionLabel: presentation.actionLabel, recommendedMovementFamily: presentation.recommendedMovementFamily, specificExerciseId: presentation.specificExerciseId };
}

...
import { UnavailableAuthTokenProvider } from '@/services/auth/auth-token-provider';
 ...
 */
export function resolveTrainerService(authTokenProvider: AuthTokenProvider): AITrainerService {
  const endpointUrl = resolveTrainerEndpointUrl();
  return endpointUrl && authTokenProvider.isAvailable
    ? new RemoteTrainerService(endpointUrl, authTokenProvider)
    : new OfflineTrainerService();
}

export const aiTrainerService: AITrainerService = resolveTrainerService(
  new UnavailableAuthTokenProvider()
);

...
/**
 * Danbaek detail viewer. There is no separate runtime player identity here: until a real approved
 * 3D Danbaek exists, this uses the same canonical parametric renderer and current BodyParameters.
 */
...
function CharacterStage({
  ...
  const panResponder = useMemo(
    () => PanResponder.create({
      ...
      onPanResponderMove: (_event, gesture) => setDragDeltaDeg(gesture.dx * CharacterRotationDegreesPerPixel),
      onPanResponderRelease: (_event, gesture) => {
        setBaseRotationDeg((previous) => normalizeRotationDeg(previous + gesture.dx * CharacterRotationDegreesPerPixel));
        setDragDeltaDeg(0);
      },

...
export default function DanbaekWorldScreen() {
  const { danbaekLearning, characterAppearance, bodyParameters } = useAppData();
  ...
  return <SubScreen title="단백세상" accent>
    <View style={styles.worldCard}>
      ...
      <View style={styles.danbaek}>
        <View pointerEvents="none" style={styles.characterStage}>
          <PlayerCharacter appearance={characterAppearance} slot="home" height={172} bodyParameters={bodyParameters} idle />
        </View>
        <ThemedText type="heading">단백이</ThemedText>
        <ThemedText type="caption" themeColor="textSecondary">{scene.state==='blocked'?'배운 동작을 따라 해보다 멈췄어요.':'배운 동작으로 끝까지 지나갔어요.'}</ThemedText>
      </View>
    </View>
...
const styles=StyleSheet.create({worldCard:{gap:Spacing.three,padding:Spacing.three,borderRadius:Radius.large,backgroundColor:Colors.dark.backgroundElement,borderWidth:1,borderColor:Colors.dark.border},path:{gap:Spacing.two},node:{padding:Spacing.two,borderRadius:Radius.medium,borderWidth:1,borderColor:Colors.dark.border},nodeCleared:{borderColor:Colors.dark.gold},nodeCurrent:{borderColor:Colors.dark.warmOrange,backgroundColor:Colors.dark.backgroundSelected},danbaek:{minHeight:238,alignItems:'center',justifyContent:'center',gap:Spacing.one},characterStage:{height:172,width:'100%',alignItems:'center',justifyContent:'center',overflow:'hidden'},resultCard:{gap:Spacing.three,padding:Spacing.three,borderRadius:Radius.large,backgroundColor:Colors.dark.backgroundElement}});

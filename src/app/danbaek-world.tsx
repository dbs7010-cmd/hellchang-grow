  ...
  return (
    <SubScreen title="단백세상" accent>
      <View style={styles.worldCard}>
        ...
        <View style={styles.path}>
          {['출발', '밀기 관문', '벤치몬스터'].map((label, index) => {
            ...
            return <View key={label} style={[styles.node, cleared && styles.nodeCleared, current && styles.nodeCurrent]}><ThemedText type="caption">{cleared ? '✓ ' : current ? '● ' : '○ '}{label}</ThemedText></View>;
          })}
        </View>
      ...
      <View style={styles.resultCard}>
        ...
        {scene.state === 'blocked' ? <PrimaryButton label={scene.actionLabel} onPress={() => router.push({ pathname: '/ai-chat', params: { movementFamily: scene.recommendedMovementFamily, exerciseId: scene.specificExerciseId ?? undefined, source: 'danbaek-world-block' } })} /> : <PrimaryButton label="홈으로 돌아가기" onPress={() => router.back()} />}
      </View>
    </SubScreen>
  );
}
...
const styles = StyleSheet.create({
  ...
  danbaek: { minHeight: 180, alignItems: 'center', justifyContent: 'center', gap: Spacing.two },

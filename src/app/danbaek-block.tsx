...
import { markDanbaekWorldWorkoutReturn } from '@/services/world/world-return';
...
export default function DanbaekBlockScreen() {
  const router=useRouter(); const theme=useTheme(); const {workoutRecords,startWorkoutSession,activeSession}=useAppData();
  ...
  const handleStart=async(exercise:QuickStartExercise)=>{if(activeSession&&activeSession.status!=='completed'){router.replace('/session');return;} await startWorkoutSession('strength',{primaryMuscleGroup:presentation?.muscleGroup,initialExercises:[exercise]}); markDanbaekWorldWorkoutReturn(); clearPendingDanbaekBlock(); router.replace('/session');};

...
- **Verification**: `npm run verify:storage`(`scripts/verify-storage-recovery.ts`, 68개) — 깨진/잘린 JSON, 배열 아닌 저장값, 프로필 필수 필드·NaN·배열, 온보딩 플래그 truthy 함정, 양방향 온보딩 경계(플래그만 있고 프로필 없음 → 온보딩 / 플래그 없고 프로필 있음 → 완료 + 플래그 보강), 부팅 실패 시 온보딩을 열지 않음, [다시 시도] 상태 전이, 손상의 키 단위 격리. `tsc` / `lint` / 기존 verify 12종도 함께 PASS(총 794개).
  ...
  - **추가(2026-08-25, commit `4968d2c`)**: `streak` / `pass` / `trainer-usage` / `referral` / `event` 다섯 키도 모양 검사를 받는다. 저장소마다 전용 검사기를 만들지 않고 공용 조각(`asStoredRecord` / `asStoredCount` / `asStoredFlag` / `asStoredDateString` / `asStoredText`)을 조합한다. 실제 위험을 확인하고 고쳤다 — streak은 저장값에 +1을 하고(`"5" + 1 = "51"`), trainer-usage는 `> 0` 비교 하나로 AI PT를 열며(`"5" > 0`도 true), event 만료 시각은 `toISOString()`에서 던질 수 있다. `verify:storage` 44개 추가(95 → 139).
  - **추가(2026-08-25, commit `8444a0f`)**: `session-completion`(pending receipt)도 닫혔다. 승인된 방침(`DEC-010`)에 따라 믿을 수 없는 receipt는 버리지도 진행하지도 않고 완료를 멈춘 뒤 [다시 시도]를 노출한다 — 다른 세션의 잔해만 예외로 통과시킨다. 이 키만은 파싱 실패를 "없는 값"으로 접지 않는다(`readRawString` + `classifyStoredReceiptRaw`). `verify:storage` 29개 추가(139 → 168). `growth`는 `migrateGrowthState()`가 이미 덮는다(`verify:growth`).
  - **남은 공백**: 저장값 모양 검사 자체는 모든 키에서 닫혔다. 남은 것은 AsyncStorage 실제 I/O와 React 배선, 그리고 실기기 kill 수동 절차(위 참조)다.
